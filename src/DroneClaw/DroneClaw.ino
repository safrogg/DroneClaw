...
#define LOW_MOTOR 1200

#define lowest(val) (val < LOW_MOTOR) ? LOW_MOTOR : val
...
void control() {
  if (drone.throttle < 1000) { // when throttle is less than 1000 disable escs
    ...
    int fl = lowest(drone.throttle - pid_pitch + pid_roll + pid_yaw);
    int fr = lowest(drone.throttle - pid_pitch - pid_roll - pid_yaw);
    int bl = lowest(drone.throttle + pid_pitch + pid_roll - pid_yaw);
    int br = lowest(drone.throttle + pid_pitch - pid_roll + pid_yaw);
    Serial.print(fr);
    Serial.print(",");
    Serial.print(br);
    Serial.print(",");
    ...
    #else
    // write the data to the servos
    servos[FR_ESC].writeMicroseconds(lowest(drone.throttle - pid_pitch - pid_roll + pid_yaw));
    servos[BR_ESC].writeMicroseconds(lowest(drone.throttle + pid_pitch - pid_roll - pid_yaw));
    servos[FL_ESC].writeMicroseconds(lowest(drone.throttle - pid_pitch + pid_roll - pid_yaw));
    servos[BL_ESC].writeMicroseconds(lowest(drone.throttle + pid_pitch + pid_roll + pid_yaw));
    #endif
  }
}
